...
const acentos: Record<Acento, { tile: string; valor: string; borde: string }> = {
  navy: { tile: "bg-navy-50 text-navy-600", valor: "text-navy-700", borde: "bg-navy-500" },
  teal: { tile: "bg-teal-50 text-teal-600", valor: "text-teal-700", borde: "bg-teal-500" },
  amber: { tile: "bg-amber-50 text-amber-600", valor: "text-amber-700", borde: "bg-amber-400" },
  riesgo: { tile: "bg-rose-50 text-rose-600", valor: "text-rose-700", borde: "bg-rose-500" },
};
...
export function KpiCard({
  ...
  return (
    <div className="group relative overflow-hidden rounded-xl border border-slate-200/80 bg-white p-4 shadow-tarjeta transition-shadow hover:shadow-elevada">
      ...
      <div className="flex items-start justify-between gap-3">
        <p className="text-etiqueta font-semibold uppercase text-slate-500">{etiqueta}</p>
        ...
        {delta && (
          <span
            className={`tabular text-xs font-medium ${
              delta.positivo ? "text-teal-600" : "text-rose-600"
            }`}
          >
            {delta.valor}
          </span>
      ...
      {detalle && <p className="mt-2 text-xs leading-relaxed text-slate-500">{detalle}</p>}

      {alerta && (
        <p className="mt-2.5 inline-flex items-center gap-1.5 rounded-md bg-rose-50 px-2 py-1 text-xs font-medium text-rose-700">
