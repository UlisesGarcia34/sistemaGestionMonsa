...
export function SegmentedControl<T extends string>({
  ...
  return (
    <div className="inline-flex rounded-xl border border-slate-200 bg-slate-100/70 p-1 shadow-tarjeta dark:border-slate-800 dark:bg-slate-900">
      {opciones.map((o) => {
        ...
        return (
          <button
            ...
            className={`flex items-center gap-1.5 rounded-lg px-3.5 py-1.5 text-sm font-medium transition-all ${
              activo
                ? "bg-white text-navy-700 shadow-tarjeta dark:bg-slate-800 dark:text-teal-300"
            ...
            {o.contador != null && (
              <span
                className={`tabular ml-1 rounded-full px-1.5 py-0.5 text-[11px] font-semibold ${
                  activo
                    ? "bg-navy-50 text-navy-700 dark:bg-teal-900/40 dark:text-teal-300"
