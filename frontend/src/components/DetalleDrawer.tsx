...
export function DetalleDrawer({
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end" role="dialog" aria-modal="true">
      ...
      <aside className="relative flex h-full w-full max-w-xl flex-col border-l border-slate-200 bg-white shadow-panel">
        ...
        <div className="flex-1 space-y-6 overflow-y-auto px-5 py-5">
          {grupos.map((grupo) => (
            <section key={grupo.titulo}>
              <h3 className="mb-2.5 text-etiqueta font-semibold uppercase text-teal-700">
                {grupo.titulo}
              </h3>
              <dl className="grid grid-cols-2 gap-x-5 gap-y-3.5">
                {grupo.campos.map((campo) => (
                  <div key={campo.label} className={campo.ancho ? "col-span-2" : ""}>
                    ...
                    <dd className="mt-0.5 break-words text-sm text-slate-800">{campo.valor}</dd>
                  </div>
                ))}
              </dl>
            </section>
